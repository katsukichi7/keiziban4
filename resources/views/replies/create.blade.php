...
<body>
    ...
    <div class="message-form">
        @if(isset($reply))
        <form method="POST" action="{{route('replies.store',['post'=>$post->id,'message'=>$message->id,'reply'=>$reply->id])}}" class="message-area">
            @else
            <form method="POST" action="{{route('replies.store',['post'=>$post->id,'message'=>$message->id])}}" class="message-area">
                @endif
                @csrf
                @error('content')
                ...
                <div>
                    ...
                    <textarea name="content" id="messageContent" rows="6" cols="90" placeholder="コメントを書く">
                >>
                </textarea>
